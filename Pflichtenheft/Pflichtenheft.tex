...
{
name={CBMC},
description={C Bounded Model Checker: Ein Programm, welches C-Programme mittels bounded model checking auf Fehler und vom Benutzer definierte Assertions untersucht}
}
...
{
name={Subset},
description={Macht nur Sinn relativ zu einem Superset. Alle Elemente des Subsets sind auch Elemente des Supersets}
}
\newglossaryentry{Quantor}
{
name={Quantor},
description={Ein Operator der Prädikatenlogik. Bindet eine Variable. Am öftesten verwendet sind Existenz- und Allquantor}
}
...
In diesem Abschnitt wird ein grober Überblick über die Sprache, welche der Eigenschafteneditor verwendet, gegeben. Es handelt sich um ein \gls{Subset} der C-Sprache zusammen mit einigen Ergänzungen. Diese werden im Folgenden erläutert. 

Formale Eigenschaften werden in Vor- und Nachbedingungen unterteilt. Diese wiederum werden vom User als eine Liste boolscher Ausdrücke angegeben. Die Sprache erlaubt folgende Konstrukte zur Konstruktion boolscher Ausdrücke:
...
\item \gls{Quantor}en für Wähler, Kandidaten und Sitze in der Form von \gls{Makro}s. Ein bisher ungenutzter Variablenname wird als Argument erwartet. Dieser kann in dem darauf folgenden Ausdruck als symbolische Variable entsprechenden Typs verwendet werden. \\
...
Ziel des Programmes ist es eine Lösung zur Analyse von formalen Eigenschaften von Wahlverfahren zu präsentieren. Zur Analyse wird dazu \gls{CBMC} verwendet. Das Programm soll auch von Nicht-Informatikern mit minimalem Aufwand erlernt und eingesetzt werden können.
...
Die Analyse der gegebenen Eigenschaften wird durch \ac{CBMC} geschehen. Aufgabe des Programmes wird sein, die Eingaben des \gls{Benutzer}s aufzubereiten, so dass \ac{CBMC} damit aufgerufen werden kann. Die Ausgabe von \ac{CBMC} wird dann vom Programm interpretiert und dem Benutzer präsentiert. 

Alles bisher Beschriebene wäre theoretisch schon ohne Verwendung des Programmes möglich. Allerdings wäre der damit verbundene Lern- und Einarbeitungsaufwand sehr hoch. Außerdem sind mit dem \gls{CBMC}-Aufruf "'von Hand"' einige Aufgaben verbunden, welche sich schneller und korrekter durch den Computer automatisieren lassen. Hinzu kommen große Anforderungen an die Informatikkenntnisse des \gls{Benutzer}s. Dies ist ein Problem, da die Erforschung von Wahlverfahren auch von Nicht-Informatikern durchgeführt wird. Daher ist ein Schwerpunkt unseres Programms einfache Benutzbarkeit, besonders für Nicht-Informatiker. Dies soll erreicht werden über eine \ac{GUI}, welche oft benötigte Funktionalität bereitstellt. Einfache syntaktische Fehler im Code sollen während des Editierens erkannt werden. Dadurch soll die Untersuchung von Wahlverfahren leichter und schneller werden, was den Mehrwert unseres Programmes ausmacht.
...
Ein Editor wird bereitgestellt, in dem formale Eigenschaften beschrieben werden können. Die Beschreibung erfolgt in der in \ref{Sprache-für-formale-Eigenschaften} beschriebenen Syntax. \\
...
\begin{itemize}
	...
	\item Listen können gespeichert und geladen werden
\end{itemize}

\subsection{Parametereditor}
Parameter für eine Wahl (Anzahl der Wähler, Kandidaten und Sitzen) können festgelegt werden. Bei der Analyse wird das Wahlverfahren nur innerhalb dieser Wahlparameter geprüft. \\
Folgende Parameter bietet der Parametereditor:
\begin{itemize}
...
Das Erstellen und Überprüfen von neuen Wahlverfahren gestaltet sich ohne Hilfsmittel als schwierig. Das Produkt soll diese Aufgabe stark vereinfachen.\\
...
Er installiert das Tool und entwickelt damit eine erste Version des Wahlverfahrens. Zusätzlich gibt er die vorgegebenen Eigenschaften als formale Eigenschaften in das Tool ein.\\
...
Das Ergebnis der Überprüfung zeigt, dass nicht alle Eigenschaften vom Wahlverfahren erfüllt werden und gibt ein Gegenbeispiel aus.\
...
Er prüft es nochmals auf alle Eigenschaften welche nun alle erfüllt werden.\\
...
Das Tool wird installiert, das Wahlverfahren im C-Editor geladen und es werden formale Eigenschaften erstellt.\\
Um das Wahlverfahren auf die Eigenschaften zu prüfen, werden Parameter zu \\Kandidaten-, Stimmen- und Wähleranzahl, sowie ein Timeout als obere Zeitgrenze für die Überprüfung angegeben.\\
Sobald das Tool mit der Überprüfung fertig ist, werden erfüllte und nicht erfüllte Eigenschaften angezeigt. Diese Informationen benutzt die Prüfstelle dann in ihrem Bericht über das Wahlverfahren.\\
...
/FM0020/ Überprüfung dieser Eigenschaften via \ac{CBMC} \\
...
\subsection{Muss-Kriterien}
/FM1010/ Darstellung aller für das Programmieren in C benötigter Zeichen \\
...
/D10/ Wahlverfahren werden vom \gls{Benutzer} in der Programmiersprache C beschrieben. Diese Beschreibungen werden in einer Datei mit vom \gls{Benutzer} gegebenen Namen und der Endung .c gespeichert. \\
...
\textbf{Beschreibung der Erweiterungen:}\\
1. Der Benutzer verändert seinen Code. \\
2. Während der Nutzer seinen Code verändert werden syntaktische Fehler erkannt wie vergessene Semicolon, nicht geschlossene Klammern und andere Konstrukte, welche der (C) C-Grammatik oder der (E) Grammatik widersprechen. \\
\end{addmargin}
C: Automatisches Einrücken des Codes in Schleifen und if-Statements 
\begin{addmargin}[15pt]{0pt}
\textbf{Beschreibung der Erweiterungen:}\\
1. Der Benutzer verändert seinen Code. \\
2. Während der Nutzer seinen Code verändert werden Zeilen in Schleifen und if-Statements erkannt und automatisch eingerückt. \\
\end{addmargin}
...
Diese Kapselung setzt voraus, dass die verschiedenen Komponenten mit den korrekten Abhängigkeiten konstruiert werden. Funktionalität hierzu befindet sich in dem Dependency-Manager. Dieser stellt eine Schnittstelle dar, über welche die Klassen Instanzen der von ihnen benötigten Komponenten beziehen können. In dem Diagramm werden dennoch die konkreten "benutzen" Beziehungen visualisiert. 

Das Kommunikator-Paket übernimmt die Kommunikation zwischen dem Parameter-Editor und den restlichen Komponenten. So wird es unter anderem der \ac{CBMC}-Schnittstelle die korrekten Daten zukommen lassen und deren Ergebnisse an die Eigenschaftenliste weiterleiten.
...
Die \ac{CBMC}-Schnittstelle erwartet die Beschreibung des Wahlverfahrens sowie die zu überprüfenden Eigenschaften (jeweils in Wrapper-Klassen). Sie erzeugt den Input für \ac{CBMC}, ruft \ac{CBMC} mit diesem Input auf und gibt das Ergebnis (in einer Wrapper-Klasse) zurück.

Diese Wrapper-Klassen erleichtern den Umgang mit den enthaltenen Daten. Die Wrapper-Klasse für \ac{CBMC}-Ergebnisse wird Funktionen zur Abfrage der Wahleingaben und Wahlergebnisse bereitstellen. Die Wrapper-Klassen für Eigenschaften und C-Code werden es erleichtern, den für \ac{CBMC} notwendigen Code zu generieren. Zusätzlich erleichtern sie eine Trennung der internen (Model) und externen (View) Darstellung der Daten. So kann die Visualisierung der \ac{CBMC}-Ergebnisse von einer weiteren Klasse übernommen werden. Diese erwartet lediglich die Ergebnis-Wrapper-Klasse sowie eine Swing-Komponente auf welcher das Ergebnis darzustellen ist. Allgemein sorgt diese zusätzliche  Abstraktion für eine flexible Architektur. Sollte das Programm in Zukunft um weitere Programmiersprachen oder einen graphischen Editor zur Beschreibung von Wahlverfahren erweitert werden, so muss lediglich die entsprechende Wrapper-Klasse korrekt erzeugt werden.
